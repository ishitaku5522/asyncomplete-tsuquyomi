if get(g:, 'loaded_autoload_asyncomplete_sources_tsuquyomi')
  finish
endif
let g:loaded_autoload_asyncomplete_sources_tsuquyomi = 1
let s:save_cpo = &cpo
set cpo&vim

" Copied from https://github.com/yami-beta/asyncomplete-omni.vim
" ORIGINAL LICENCE: MIT
" ORIGINAL AUTHOR: Takahiro Abe
function! asyncomplete#sources#tsuquyomi#get_source_options(opts) abort
  return extend({
        \ 'refresh_pattern': '\%(\k\|\.\)',
        \}, a:opts)
endfunction

" Forked from https://github.com/yami-beta/asyncomplete-omni.vim
" ORIGINAL LICENCE: MIT
" ORIGINAL AUTHOR: Takahiro Abe
...
function! asyncomplete#sources#tsuquyomi#completor(opt, ctx) abort
  try
    ...
    call asyncomplete#log('tsuquyomi', 'error', v:exception)
  endtry
endfunction
